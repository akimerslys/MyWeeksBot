...
from bot.keyboards.inline.guide import guide_start_kb, start_menu_kb, new_lang_kb
...
async def start_message_with_args(message: Message, bot: Bot, command: CommandObject, session: AsyncSession,
    ...
    try:
        ...
        return
    if not await user_exists(session, message.from_user.id) or not await get_user_active(session, message.from_user.id):
        await bot.send_message(message.from_user.id,
                               _("Got notification\nDate: {date} ({tz})\nText: {text}\n\n<i>Complete the registration to "
                                 "add it!</i>").format(date=notif_date, tz=notif_args[1], text=shared_notif.text)
                               )
        ...
        await bot.send_message(message.from_user.id, _("Notification Added!"))
...
async def set_new_lang(call: F.CallbackQuery, bot: Bot, session: AsyncSession, state: FSMContext):
    ...
    if not await user_exists(session, call.from_user.id):
        await add_user(session, call.from_user.id, call.from_user.first_name, lang)
    else:
        await set_language_code(session, call.from_user.id, lang)
...
async def set_new_lang(call: F.CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    await call.answer(_("tz_changed_to{}").format(tz))
...
async def guide_pg_1(call: F.CallbackQuery, bot: Bot):
    ...
    try:
        ...
        await bot.send_message(call.from_user.id, _("ERROR_MSG"))
        return
...
async def guide_complete(call: F.CallbackQuery, bot: Bot, state: FSMContext):
    await call.message.delete()
    await call.answer(_("Thank you for Completing guide"))
...
async def complete_user_reg(call: F.CallbackQuery, session: AsyncSession, bot: Bot, state: FSMContext):
    id = await bot.send_message(call.from_user.id, _("WAIT_MSG"))
    data = await state.get_data()
    if await user_exists(session, call.from_user.id):
        await set_user_active(session, call.from_user.id, True)
        await set_language_code(session, call.from_user.id, data.get('lang'))
        await set_timezone(session, call.from_user.id, data.get('tz'))
    else:
        await add_user(session, call.from_user.id, call.from_user.first_name, data.get('lang'), data.get('tz'))
    ...
    if data.get('notif_id'):
        ...
        await bot.send_message(call.from_user.id, _("Notification Added!"))
    await state.clear()
    await call.answer(_("Registration completed!"))
