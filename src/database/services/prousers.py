...
from src.database.models import ProUserModel
...
"""
    ...
    tournaments: Mapped[list] = mapped_column(default=None, nullable=True)
...
async def user_exists(session: AsyncSession, user_id: int) -> bool:
    ...
    query = select(ProUserModel.id).filter_by(user_id=user_id).limit(1)
...
async def user_logged(session: AsyncSession, user_id: int) -> bool:
    ...
    query = select(ProUserModel.active).filter_by(user_id=user_id).limit(1)
...
async def get_user_active(session: AsyncSession, user_id: int) -> bool:
    query = select(ProUserModel.active).filter_by(user_id=user_id)
...
async def set_user_active(session: AsyncSession, user_id: int, active: bool) -> None:
    stmt = update(ProUserModel).where(ProUserModel.user_id == user_id).values(active=active)
...
async def get_user(session: AsyncSession, user_id: int) -> ProUserModel:
    query = select(ProUserModel).filter_by(user_id=user_id)
...
async def get_first_name(session: AsyncSession, user_id: int) -> str:
    query = select(ProUserModel.first_name).filter_by(user_id=user_id)
...
async def get_language_code(session: AsyncSession, user_id: int) -> str:
    query = select(ProUserModel.language_code).filter_by(user_id=user_id)
...
async def set_language_code(
    ...
    stmt = update(ProUserModel).where(ProUserModel.user_id == user_id).values(language_code=language_code)
    await session.execute(stmt)
    await session.commit()
...
async def get_timezone(session: AsyncSession, user_id: int) -> str:
    query = select(ProUserModel.timezone).filter_by(user_id=user_id)
...
async def set_timezone(
    ...
    stmt = update(ProUserModel).where(ProUserModel.user_id == user_id).values(timezone=timezone)
...
async def is_premium(session: AsyncSession, user_id: int) -> bool:
    query = select(ProUserModel.is_premium).filter_by(user_id=user_id)
...
async def set_user_premium(session: AsyncSession, user_id: int, days: int) -> None:
    ...
    if await is_premium(session, user_id):
        stmt = update(ProUserModel).where(ProUserModel.user_id == user_id).values(
            premium_until=ProUserModel.premium_until + timedelta(days=days))
    else:
        stmt = update(ProUserModel).where(ProUserModel.user_id == user_id).values(
...
async def get_all_users(session: AsyncSession) -> list[ProUserModel]:
    query = select(ProUserModel)
...
async def count_users(session: AsyncSession) -> int:
    query = select(func.count()).select_from(ProUserModel)
...
async def block_user(session: AsyncSession, user_id: int) -> None:
    ...
    stmt = update(ProUserModel).where(ProUserModel.user_id == user_id).values(is_blocked=True)
...
async def unblock_user(session: AsyncSession, user_id: int) -> None:
    ...
    stmt = update(ProUserModel).where(ProUserModel.user_id == user_id).values(is_blocked=False)
...
async def is_blocked(session: AsyncSession, user_id: int) -> bool:
    query = select(ProUserModel.is_blocked).filter_by(user_id=user_id)
...
async def delete_user(session: AsyncSession, user_id: int) -> None:
    ...
    stmt = delete(ProUserModel).where(ProUserModel.user_id == user_id)
