...
def timezone_simple_keyboard(user_active: bool = False) -> InlineKeyboardMarkup:
    tmp = 'set_tz_'
    if user_active:
        tmp = 'change_tz_'
    buttons = [
        ...
        [InlineKeyboardButton(text=_("timezone_by_location") + " (beta)", callback_data=f"timezone_send_geo")],
...
def timezone_advanced_keyboard(user_active: bool = False, first_page: bool = True) -> InlineKeyboardMarkup:
    ...
    if first_page:
        for i in range(11):
            builder.button(text=f"GMT+{i + 1}", callback_data=f"{tmp}Etc/GMT+{i + 1}")

        builder.button(text=f"More...", callback_data="timezone_show_adv_2")

    else:
        for i in range(11):
            builder.button(text=f"GMT{i - 12}", callback_data=f"{tmp}Etc/GMT{i - 12}")
...
def timezone_country_list_kb(tz_list: list, user_active: bool) -> InlineKeyboardMarkup:
    calldata = "set_tz_"
    if user_active:
        calldata = "change_tz_"
